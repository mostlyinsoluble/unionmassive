    ...
    TextSpan containingLocationForStringOrComment = default) : IEquatable<RenameLocation>
{
    ...
    internal static bool ShouldRename(CandidateReason candidateReason)
    {
        if (candidateReason != CandidateReason.None)
        {
            ...
            {
                CandidateReason.NotATypeOrNamespace => false,// We had a reference to the symbol in a location where we needed a 
                                                             ...
                                                             // renamed.
                CandidateReason.NotAnEvent or CandidateReason.NotAWithEventsMember => false,// it's unlikely that someone would be referencing a non-event in an 
                                                                                            // event context.  Likely this location should not be included.
                CandidateReason.NotAnAttributeType => true,// It's feasible that someone was referencing some type in an attribute
                                                           // location before making that type itself descend from System.Attribute.
                                                           // Still allow this type to be renamed.
                CandidateReason.WrongArity => true,// Someone may have provided the wrong number of type arguments to
                                                   // a type/method when calling it. We should still allow the reference
                                                   // to be updated.
                CandidateReason.NotCreatable => true,// Can happen when someone tries to do something like 'new' an 
                                                     // abstract type.  We still want to allow renaming this location.
                CandidateReason.NotReferencable => true,// Happens when the user does something like directly accessing
                                                        // the accessor of a normal property.  In this case, we do still
                                                        // want to allow the rename to happen.
                CandidateReason.Inaccessible => true,// Can trivially occur in code that is in an initially broken state
                                                     // where inaccessible members are being referenced.  We still want
                                                     // to update these references.
                CandidateReason.NotAValue or CandidateReason.NotAVariable => false,// Happens with code like "NS = 1".  If "NS" is binding now to a 
                ...
                CandidateReason.NotInvocable => true,// Happens when something like a variable is being invoked, but the variable
                ...
                CandidateReason.StaticInstanceMismatch => true,// Similar to 'Inaccessible', the code is currently broken, but it's fairly
                                                               // clear what the user's intent was.  In this case, we want to update the
                                                               // references, even though the code isn't valid.
                CandidateReason.OverloadResolutionFailure => false,// Here we were renaming a method, and have a reference location that didn't
                ...
                CandidateReason.LateBound => false,// This is a late bound call, so we should not update this location. 
                CandidateReason.Ambiguous => false,// We should not touch ambiguous code.  We have no way to feel confident that
                                                   // this really is a location that we should be updating.
                CandidateReason.MemberGroup => true,// MemberGroup is not an error case.  It happens in completely legal code,
                                                    // like nameof(int.ToString).  Because of that, we do want to update the
                                                    // reference here.
                _ => false,// For cases added in the future, conservatively presume we can't update them.
