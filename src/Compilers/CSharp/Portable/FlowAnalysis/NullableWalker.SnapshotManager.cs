        ...
        internal sealed class SnapshotManager
        {
            ...
            internal void VerifyUpdatedSymbols()
            {
                foreach (var ((expr, originalSymbol), updatedSymbol) in _updatedSymbolsMap)
                {
                    ...
                    RoslynDebug.Assert(originalSymbol is not null, $"Recorded null original symbol for {debugText}");
                    RoslynDebug.Assert(updatedSymbol is not null, $"Recorded null updated symbol for {debugText}");
