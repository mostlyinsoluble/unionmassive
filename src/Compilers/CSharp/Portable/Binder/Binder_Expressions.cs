    ...
    internal partial class Binder
    {
        ...
        private void CheckContextForPointerTypes(ExpressionSyntax node, BindingDiagnosticBag diagnostics, BoundExpression expr)
        {
            if (!expr.HasAnyErrors && !IsInsideNameof)
            {
                ...
                if (exprType is not null && exprType.ContainsPointerOrFunctionPointer())
                {
...
#nullable enable
        ...
        private BoundExpression BindDeclarationExpressionAsError(DeclarationExpressionSyntax node, BindingDiagnosticBag diagnostics)
        {
            ...
            bool isConst = false;
            var declType = BindVariableTypeWithAnnotations(node.Designation, diagnostics, node.Type.SkipScoped(out _).SkipRef(), ref isConst, out bool isVar, out AliasSymbol alias);
        ...
        private BoundExpression BindTupleExpression(TupleExpressionSyntax node, BindingDiagnosticBag diagnostics)
        {
            ...
            if (hasNaturalType)
            {
                tupleType = NamedTypeSymbol.CreateTuple(node.Location, elements, locations, elementNames,
                    this.Compilation, syntax: node, diagnostics: diagnostics, shouldCheckConstraints: true,
                    includeNullability: false, errorPositions: default);
            }
            else
            {
            ...
            return new BoundTupleLiteral(node, boundArguments.ToImmutableAndFree(), elementNames, inferredPositions, tupleType, hasErrors);
        }
            ...
            SeparatedSyntaxList<ArgumentSyntax> arguments, BindingDiagnosticBag diagnostics)
        {
            ...
            for (int i = 0; i < numElements; i++)
            {
                ...
                else
                {
                    ...
                    if (diagnostics != null && !CheckTupleMemberName(name, i,
                        argumentSyntax.NameColon?.Name, diagnostics, uniqueFieldNames))
                        hasErrors = true;
                }
        ...
        {
            if (inferredElementNames is null)
                return;
        ...
        private static string? InferTupleElementName(SyntaxNode syntax)
        {
            ...
            if (name is null || NamedTypeSymbol.IsTupleElementNameReserved(name) != -1)
            {
                return null;
            }
        ...
        private BoundExpression BindArgList(CSharpSyntaxNode node, BindingDiagnosticBag diagnostics)
        {
            ...
            if (method is null || !method.IsVararg)
            {
...
#nullable enable
        ...
        internal void ReportFieldContextualKeywordConflictIfAny(Symbol? symbol, SyntaxNode syntax, SyntaxToken identifier, BindingDiagnosticBag diagnostics)
        {
            ...
            if (name == "field" && ContainingMember() is MethodSymbol { MethodKind: MethodKind.PropertyGet or MethodKind.PropertySet, AssociatedSymbol: PropertySymbol { IsIndexer: false } })
            {
                diagnostics.Add(ErrorCode.ERR_VariableDeclarationNamedField, syntax);
            }
        }
...
#nullable disable
            ...
            BindingDiagnosticBag diagnostics)
        {
            ...
            if (lookupResult.Kind != LookupResultKind.Empty)
            {
                ...
                Symbol symbol = GetSymbolOrMethodOrPropertyGroup(lookupResult, node, name, node.Arity, members, diagnostics, out bool isError, qualifier: null);  // reports diagnostics in result.
        ...
        private static bool FallBackOnDiscard(IdentifierNameSyntax node, BindingDiagnosticBag diagnostics)
        {
            ...
            return node.GetContainingDeconstruction() != null || IsOutVarDiscardIdentifier(node);
        }
        ...
        private bool IsBadLocalOrParameterCapture(Symbol symbol, TypeSymbol type, RefKind refKind)
        {
            if (refKind != RefKind.None || type.IsRestrictedType())
            {
                ...
                if (containingMethod is not null && (object)symbol.ContainingSymbol != (object)containingMethod)
                {
                ...
                case SymbolKind.Local:
                    {
                        ...
                        TypeSymbol type = BindResultTypeForLocalVariableReference(node, localSymbol, diagnostics, out bool isNullableUnknown, out bool isTypeError);
                        isError |= isTypeError;
...
#nullable disable
        ...
        private BoundBaseReference BindBase(BaseExpressionSyntax node, BindingDiagnosticBag diagnostics)
        {
            ...
            else if (baseType is null) // e.g. in System.Object
            {
        ...
        private BoundExpression BindExplicitNullableCastFromNonNullable(ExpressionSyntax node, BoundExpression operand, TypeWithAnnotations targetTypeWithAnnotations, BindingDiagnosticBag diagnostics)
        {
            ...
            Debug.Assert(operand.Type is not null && !operand.Type.IsNullableType());
        ...
        private void BindArgumentsAndNames(BaseArgumentListSyntax argumentListOpt, BindingDiagnosticBag diagnostics, AnalyzedArguments result, bool allowArglist = false, bool isDelegateCreation = false)
        {
            ...
            {
                BindArgumentAndName(result, diagnostics, ref hadError, argumentSyntax, allowArglist, isDelegateCreation: isDelegateCreation);
            }
        }
            ...
            BindingDiagnosticBag diagnostics)
        {
            ...
            try
            {
                ...
                if (isBaseConstructorInitializer)
                {
                    ...
                    Debug.Assert(initializerType is not null ||
                    containingType.SpecialType == SpecialType.System_Object ||
                        containingType.IsInterface);

                    if (initializerType is null || containingType.SpecialType == SpecialType.System_Object) //e.g. when defining System.Object in source
                    {
...
#nullable enable
        private BoundExpression BindCollectionExpression(CollectionExpressionSyntax syntax, BindingDiagnosticBag diagnostics, int nestingLevel = 0)
        {
            ...
            static BoundNode bindSpreadElement(SpreadElementSyntax syntax, BindingDiagnosticBag diagnostics, Binder @this)
            {
                ...
                bool hasErrors = !@this.GetEnumeratorInfoAndInferCollectionElementType(syntax, syntax.Expression, ref expression, isAsync: false, isSpread: true, diagnostics, inferredType: out _, out ForEachEnumeratorInfo.Builder builder) ||
...
#nullable disable
            ...
            BoundObjectOrCollectionValuePlaceholder implicitReceiver)
        {
                ...
                BoundObjectOrCollectionValuePlaceholder implicitReceiver)
            {
                ...
                Debug.Assert(implicitReceiver.Type is not null);
...
#nullable disable
        ...
        private bool IsConstructorAccessible(MethodSymbol constructor, ref CompoundUseSiteInfo<AssemblySymbol> useSiteInfo, bool allowProtectedConstructorsOfBaseType = false)
        {
            ...
            {
                Debug.Assert(this.Compilation.Assembly is not null);
            ...
            bool wasTargetTyped = false)
        {
            ...
            if (analyzedArguments.HasDynamicArgument)
            {
                ...
                {
                    var finalApplicableCandidates = GetCandidatesPassingFinalValidation(node, overloadResolutionResult, receiverOpt: null, default, isExtensionMethodGroup: false, diagnostics);
        ...
        {
            Debug.Assert(interfaceType is not null);
            Debug.Assert(interfaceType.IsInterfaceType());
            Debug.Assert(coClassType is not null);
            ...
            LookupResultKind lookupKind)
        {
            if (symbols.Length > 0 && symbols[0].Kind == SymbolKind.Method)
            {
                ...
                foreach (var s in symbols)
                {
                    ...
                    if (m is not null) builder.Add(m);
                }
            ...
            return new BoundBadExpression(
                node,
                lookupKind,
                symbolOpt is null ? ImmutableArray<Symbol>.Empty : ImmutableArray.Create(symbolOpt),
...
#nullable enable
        ...
        /// </summary>
        private Symbol GetSymbolOrMethodOrPropertyGroup(LookupResult result, SyntaxNode node, string plainName, int arity, ArrayBuilder<Symbol> methodOrPropertyGroup, BindingDiagnosticBag diagnostics, out bool wasError, NamespaceOrTypeSymbol? qualifier)
        {
            ...
            Debug.Assert(methodOrPropertyGroup.Any() || (other is not null));
            ...
            methodOrPropertyGroup.Clear();
            return ResultSymbol(result, plainName, arity, node, diagnostics, false, out wasError, qualifier);
        }
        ...
        private BoundExpression BindElementOrIndexerAccess(ExpressionSyntax node, BoundExpression expr, AnalyzedArguments analyzedArguments, bool allowInlineArrayElementAccess, BindingDiagnosticBag diagnostics)
        {
            ...
            {
                int? constIndex = InferConstantIndexFromSystemIndex(Compilation, convertedIndex, length, out SyntaxNode location);
...
#nullable disable
            ...
            BindingDiagnosticBag diagnostics)
        {
            ...
            if (analyzedArguments.HasDynamicArgument && overloadResolutionResult.HasAnyApplicableMember)
            {
                ...
                var finalApplicableCandidates = GetCandidatesPassingFinalValidation(syntax, overloadResolutionResult, receiver, default, isExtensionMethodGroup: false, diagnostics);
            ...
            BindingDiagnosticBag diagnostics)
        {
            ...
            else
            {
                ...
                this.CheckAndCoerceArguments<PropertySymbol>(syntax, resolutionResult, analyzedArguments, diagnostics, receiver, invokedAsExtensionMethod: false, out ImmutableArray<int> argsToParams);
